...
export function DashboardShell({
  ...
  return (
    <div className="min-h-screen bg-background">
      <header className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4 border-b border-gold/15 bg-primary px-4 py-4 text-primary-foreground md:px-8">
        ...
        <div className="flex gap-1">
          <Button
            ...
            size="icon"
            aria-label="Search"
            className="hover:bg-gold/15 hover:text-gold"
          >
            <Search />
          </Button>
          <Button
          ...
          <Button
            variant="ghost"
            size="icon"
            className="hover:bg-gold/15 hover:text-gold md:hidden"
            aria-label="Menu"
          >
            <Menu />
          </Button>
...
export function Metric({ label, value, note }: { label: string; value: string; note: string }) {
  return (
    <div className="rounded-2xl bg-card p-6 ring-1 ring-border shadow-[0_10px_30px_-16px_rgba(15,43,32,0.15)]">
