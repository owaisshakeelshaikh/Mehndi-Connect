...
function Logo({ compact = false }: { compact?: boolean }) {
  return (
    <Link href="/" className="flex min-w-0 shrink-0 items-center gap-2.5">
      <span className="flex size-9 items-center justify-center rounded-lg border border-gold/40 bg-gold/10 text-gold">
      ...
      <span className="leading-none">
        ...
        <span
          className={cn(
            "block font-sans font-semibold tracking-[0.34em] text-gold",
        ...
        floating
          ? cn(
              ...
              overlay
                ...
                : "border-gold/15 bg-primary/85 backdrop-blur-md",
            )
          : "inset-x-0 top-0 rounded-b-2xl border-x-0 border-t-0 border-b border-gold/15 bg-primary/95 backdrop-blur-xl",
      )}
    >
      <div
        className={cn(
      ...
      >
        ...
        <nav className="mx-auto hidden items-center gap-8 lg:flex" aria-label="Primary navigation">
          {nav.map(([label, to]) => (
            <Link
              key={to}
              href={to}
              className="link-underline text-[10px] font-bold uppercase tracking-[0.2em] text-primary-foreground/80 transition-colors duration-300 hover:text-gold"
            >
              {label}
            </Link>
          ))}
        ...
        <div className="flex shrink-0 items-center gap-2 sm:gap-4">
          <Link
            href="/account"
            className="link-underline hidden text-[10px] font-bold uppercase tracking-[0.2em] text-primary-foreground/80 transition-colors hover:text-gold xl:block"
          >
            Sign in
          </Link>
          <Button
          ...
          <Button
            variant="ghost"
            size="icon"
            className="hover:bg-gold/15 hover:text-gold lg:hidden"
      ...
      <nav
        ...
        className={cn(
          "grid overflow-hidden border-gold/15 bg-primary/95 backdrop-blur-xl transition-all duration-500 ease-editorial lg:hidden",
      ...
      >
        <div className="min-h-0 overflow-hidden px-4">
          ...
          <div className="flex gap-3 border-b border-primary-foreground/10 py-4">
            <Button asChild variant="gold" size="sm" className="flex-1 font-semibold">
...
export function SiteFooter() {
  ...
  return (
    <footer className="border-t border-gold/15 bg-primary px-5 pb-28 pt-16 text-primary-foreground md:px-8 md:pb-8">
        ...
        <div className="grid gap-12 border-b border-primary-foreground/10 pb-12 lg:grid-cols-[1.1fr_2fr]">
            ...
            <div className="mt-6 flex gap-3">
              {[Instagram, Facebook].map((Icon, i) => (
                <a
                  ...
                  className="flex size-9 items-center justify-center rounded-full border border-primary-foreground/15 text-primary-foreground/70 transition-colors hover:border-gold hover:text-gold"
                >
              ...
              <a
                ...
                className="flex size-9 items-center justify-center rounded-full border border-primary-foreground/15 text-[10px] font-bold text-primary-foreground/70 transition-colors hover:border-gold hover:text-gold"
              >
                Pi
              </a>
            </div>
          ...
          <div className="grid grid-cols-2 gap-10 md:grid-cols-4">
            {groups.map(([title, items]) => (
              <div key={title}>
                <p className="mb-5 text-[10px] font-bold uppercase tracking-[0.22em] text-gold">
                  {title}
                </p>
